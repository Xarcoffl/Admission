...
const chartTooltipFormatter = (value: string | number, name: string) => [value, name === 'value' ? 'Count' : name];
    ...
    <div>
      ...
      <div className="grid-2 section">
        <div className="card chart-card">
          ...
          <ResponsiveContainer width="100%" height={280}>
            <LineChart data={dailyApplications} margin={{ top: 24, right: 32, left: 0, bottom: 40 }}>
              <CartesianGrid strokeDasharray="3 3" opacity={0.2} />
              <XAxis dataKey="date" tick={{ fill: '#cbd5e1' }} angle={-45} textAnchor="end" height={80} />
              <YAxis tick={{ fill: '#cbd5e1' }} />
              <Tooltip contentStyle={chartTooltipStyle} formatter={chartTooltipFormatter} labelStyle={{ color: '#94a3b8' }} itemStyle={{ color: '#e2e8f0' }} wrapperStyle={{ zIndex: 99 }} cursor={{ fill: 'rgba(79, 139, 255, 0.1)' }} />
              <Line dataKey="applications" stroke="#4f8bff" strokeWidth={3} dot={{ r: 4 }} activeDot={{ r: 6 }} isAnimationActive />
            </LineChart>
          </ResponsiveContainer>
        </div>
        ...
        <div className="card chart-card">
          ...
          <ResponsiveContainer width="100%" height={280}>
            <BarChart data={funnelData} margin={{ top: 24, right: 32, left: 0, bottom: 40 }}>
              <CartesianGrid strokeDasharray="3 3" opacity={0.2} />
              <XAxis dataKey="name" tick={{ fill: '#cbd5e1' }} angle={-45} textAnchor="end" height={80} />
              <YAxis tick={{ fill: '#cbd5e1' }} />
              <Tooltip contentStyle={chartTooltipStyle} formatter={chartTooltipFormatter} labelStyle={{ color: '#94a3b8' }} itemStyle={{ color: '#e2e8f0' }} wrapperStyle={{ zIndex: 99 }} cursor={{ fill: 'rgba(79, 139, 255, 0.1)' }} />
              <Bar dataKey="value" fill="#34d399" radius={[10, 10, 0, 0]} label={{ position: 'top', fill: '#e2e8f0' }} isAnimationActive>
      ...
      <div className="grid-2 section">
        <div className="card chart-card">
          ...
          <ResponsiveContainer width="100%" height={280}>
            <BarChart data={schoolData} layout="vertical" margin={{ top: 12, right: 32, left: 12, bottom: 12 }}>
              <CartesianGrid strokeDasharray="3 3" opacity={0.2} />
              <XAxis type="number" tick={{ fill: '#cbd5e1' }} />
              <YAxis dataKey="name" type="category" tick={{ fill: '#cbd5e1' }} width={130} />
              <Tooltip contentStyle={chartTooltipStyle} formatter={chartTooltipFormatter} labelStyle={{ color: '#94a3b8' }} itemStyle={{ color: '#e2e8f0' }} wrapperStyle={{ zIndex: 99 }} cursor={{ fill: 'rgba(79, 139, 255, 0.1)' }} />
              <Bar dataKey="value" fill="#4f8bff" radius={[10, 10, 0, 0]} label={{ position: 'right', fill: '#e2e8f0' }} isAnimationActive />
            </BarChart>
          </ResponsiveContainer>
        </div>
        ...
        <div className="card chart-card">
          ...
          <ResponsiveContainer width="100%" height={280}>
            <PieChart>
              ...
              <Tooltip contentStyle={chartTooltipStyle} formatter={chartTooltipFormatter} labelStyle={{ color: '#94a3b8' }} itemStyle={{ color: '#e2e8f0' }} wrapperStyle={{ zIndex: 99 }} />
            </PieChart>
          </ResponsiveContainer>
        </div>
      </div>
